...
import { useIssueCreate, useIssueUpdate } from "@/features/issue-management";
import { Input } from "../ui/input";
import { Button } from "../ui/button";
import { Alert } from "../ui/alert";
import { useTheme } from "@/components/theme/ThemeProvider";
import { IssueFormDataSchema } from "@/schemas/issueSchema";
import { toast } from "@/hooks/use-toast";
import { Select, SelectContent, SelectItem, SelectTrigger } from "../ui/select";
import { IssueListType } from "@/schemas/issueSchema";
...
export function IssueForm(
    ...
    const { mutate: createIssue, isError: createError, message: createMessage } = useIssueCreate();
    const { mutate: updateIssue, isError: updateError, message: updateMessage } = useIssueUpdate();

    const { isPending, data: responseData, error }: { isPending: boolean; data: IssueListType; error: any } = useProjectFind(
        {
            searchTerm: '',
            page: 1,
            pageSize: 100000,
    ...
    console.log("🚀 ~ file: IssueForm.tsx:38 ~ responseData:", responseData)

    const [formData, setUpdatedDataLocal] = useState<IssueFormDataSchema>({
        id: updatedData?.id || "",
                ...
                <div>
                    ...
                    >
                        ...
                        <SelectContent>
                            ...
                            {/* Issue items - each with a non-empty value */}
                            {responseData.items.map((issue) => (
